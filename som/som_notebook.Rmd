...
## Basic SOM
Weźmy pod uwagę wszystkie próby rzutów wykonane przez zawodników.
```{r basic SOM}
...
W tym przypadku każdy z graczy jest tak uwzględniony na mapie, jak blisko ich linii statystyk są reprezentatywne wektory.
```{r plotting points}
par(mfrow = c(1, 2))
...
# PS: nie wiem jaki był błąd, bo ładnie mi wszystko przeszło :p ale stosowałam się dokładnie do tutorialu i poprawiłam też poprzednie linie gdy im czegoś brakowało.
...
#NBA.SOM4.hc <- cutree(hclust(dist(NBA.SOM4$codes$Y)), 5)
#add.cluster.boundaries(NBA.SOM4, NBA.SOM4.hc)
# PS: tu tez przeszło (gdy przenioslam te dwie linijki vezposrednio pod plot, bo ewidentnie dogyczą rysowania tego wykresu)
```
...
<!--```{r} -->
#to juz prawdopodobnie jest niepotrzebne
...
PS:  zrobione, wszytsko przeszło już bez błędów
